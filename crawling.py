...
def crawling_review(driver):
    json_review_list = []
    like_list = []
    processing_json_review_list = []
    proc_stop_word_review_list = []
    proc_like_review_list= []
    for i in range(50):
        try:  # 더보기 버튼이 있으면 클릭
    ...
    for review in reviews:
        ...
        for review_element in review_elements:
            ...
            if not have_stop_word(review_text) and len(review_text) > 20 and len(proc_stop_word_review_list) < 15: # 불용어를 가지고 있지 않고, 20자 이상인 경우
                json_review_list.append({"review": review_text})
                proc_stop_word_review_list.append(review_text)

        #print(f"불용어 처리 후 후기 수:{len(review_list)}")
        for i in range(len(json_review_list)):
            if like_list[i] > 10 and len(processing_json_review_list) <= 30:
                processing_json_review_list.append(json_review_list[i])
                proc_like_review_list.append(proc_stop_word_review_list[i])
        print(f"불용어 처리 + 일정 공감 수 이상 처리 후기 수:{len(processing_json_review_list)}")

        for proc_like_review in proc_like_review_list:
            print(proc_like_review)

    processing_json_review_list = json.dumps(processing_json_review_list, ensure_ascii=False, indent=4)
